    ...
    if (evt.target.readyState == FileReader.DONE) { // DONE == 2
        ...
        $.ajax({
            ...
            success: function (data, status) {
                ...
                var timeDelta = Date.now() - uploadStart;
                var timeSeconds = Math.floor(timeDelta / 1000);
                var kPerSec = ((bytesUploaded / timeSeconds) / 1024).toFixed(0)

                $("#fileUploadProgress").text(percentComplete + " % (" + kPerSec + "k/sec)");
                uploadFileInBlocks();
            },
...
function DoUpload() {
    var url = "http://localhost:7071/api/GetTempUploadUrl";
    // alert(selectedFile.name);
    var postBody = JSON.stringify({ FileName: selectedFile.name });

    $.post(
        url,
        postBody,
        function (response) {  
            $("#uploadUrl").text(response.url);
            submitUri = response.url;
